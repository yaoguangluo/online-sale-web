...
if(""==$_GET['currentPage'])
{
   $_GET['currentPage']=0;
        ...
        $link = "http://".$CFG['main']['webSite']['backendIp'].$CFG['backend']['pay']['getPayInfoByGid']."?key=".$_GET['key']."&currentPage=".$_GET['currentPage']; 
        $ch = curl_init("http://".$CFG['main']['webSite']['backendIp'].$CFG['backend']['pay']['getPayInfoByGid']."?key=".$_GET['key']."&currentPage=".$_GET['currentPage']);  
...
if(1==$output->{'status'})
        ...
        $smarty->display('cart/payInfo.mnb'); 
        }else{
               //  $smarty->assign("link",$encode);
